...
const CURSOR_BACK = '\b';
const EMPTY_SELECTED_CHARACTER = ' ';
const MOVE_CURSOR_AND_DELETE_LAST = `${CURSOR_BACK}${EMPTY_SELECTED_CHARACTER}${CURSOR_BACK}`;
...
function removeLastCharacters(charactersToRemove: number) {
  // backing up only one character moves the cursor onto the character but does not remove the previous character
  [...new Array(charactersToRemove)].map(() => terminal.write(MOVE_CURSOR_AND_DELETE_LAST));
}

function moveCursorLeft(spacesToMove: number) {
  // !! we need to stop the cursor moving if it hits the beginning of the prompt
  [...new Array(spacesToMove)].map(() => terminal.write(LEFT_ARROW));
}
